...
Packaged start **skips** any key window. Berk sets the talk secret on the hosted server (`JARVIS_HOSTED_TALK_URL`, default `https://berkkarabacak.com/jarvis`) or injects `JARVIS_OPERATOR_OPENROUTER_KEY` in the private build env. Users never see it. No notepad, no console paste.
